...
function onWindowResize()
{
    ...
    div = get("hand1");
    spacing = min((window.innerWidth - 215 - 100 - 5)/(hand1.length - 1), 110);
...
function mouseUp(e) {
    ...
    if(curDragNode != null)
    {
        if(overLow(e,get("hand2spoiler")))
            attachHand2(curDragNode.id);
        else if(overHigh(e,get("hand1spoiler")))
            attachHand1(curDragNode.id);
        else if(overLow(e,get("deck2")))
            attachDeck2(curDragNode.id);
        else if(overHigh(e,get("deck1")))
            attachDeck1(curDragNode.id);
        else if(overLow(e,get("grave2")))
            attachGrave2(curDragNode.id);
        else if(overHigh(e,get("grave1")))
