...
function _exchange()
{
    xhr.open("post","../mtg.php", true);
    xhr.setRequestHeader("Content-type","application/x-www-form-urlencoded");
    var data = "type=exchange&user=u1&";
    if(pageid.length == 0)data = "type=init&user=u1&"
...
xhr.onreadystatechange = function(e)
{
    if(xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200)
    {
        ...
        if(type != "fail")
        {
            ...
            if(rt[0][0] == 'u2' || type == 'init'){
                deck1 = rt[0].split('\r\n').slice(2);
                removeEmpty(deck1);
            }
            if(rt[1][0] == 'u2' || type == 'init'){
                deck2 = rt[1].split('\r\n').slice(2);
                removeEmpty(deck2);
            }
            if(rt[2][0] == 'u2' || type == 'init'){
                hand1 = rt[2].split('\r\n').slice(2);
                removeEmpty(hand1);
            }
            if(rt[3][0] == 'u2' || type == 'init'){
                hand2 = rt[3].split('\r\n').slice(2);
                removeEmpty(hand2);
            }
            if(rt[4][0] == 'u2' || type == 'init'){
                grave1 = rt[4].split('\r\n').slice(2);
                removeEmpty(grave1);
            }
            if(rt[5][0] == 'u2' || type == 'init'){
                grave2 = rt[5].split('\r\n').slice(2);
                removeEmpty(grave2);
            }
